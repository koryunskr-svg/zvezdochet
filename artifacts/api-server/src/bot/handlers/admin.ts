  ...
  await bot.sendMessage(chatId, "⚙️ *Панель администратора*\nВыберите действие: ", {
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: "⚙️ Настройки", callback_data: "admin_free_limit" }],
...
export async function handleAdminCallback(
 ...
 return;
    ...
    await bot.sendMessage(chatId, "⚙️ *Настройки*\n\n_Здесь будут настройки бесплатных лимитов_\n\nФункция в разработке 🔜", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "🔙 Назад", callback_data: "admin_back" }]] } });
    return;
 } else if (data === "admin_subscribers") {
    logger.info({ telegramId }, "DEBUG: Entered admin_subscribers handler");
    const db = getDb();
    await bot.answerCallbackQuery(query.id);
    ...
    for (const admin of admins) {
      ...
      if (admin.telegram_id !== telegramId) {
        keyboard.push([{ text: `❌ ${admin.telegram_id}${isSuper}`, callback_data: `admin_remove_confirm_${admin.telegram_id}` }]);
      }
    }
